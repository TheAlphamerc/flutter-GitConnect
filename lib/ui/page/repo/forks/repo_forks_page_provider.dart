...
class RepoForksPageProvider extends StatefulWidget {
  final String name;
  final String owner;
  const RepoForksPageProvider({Key key, this.name, this.owner})
      : super(key: key);
      ...
      builder: (context) {
        return BlocProvider<RepoBloc>(
          create: (BuildContext context) => RepoBloc()
            ..add(LoadForksEvent(name: name, owner: owner, count: count)),
  ...
  @override
  _RepoForksPageProviderState createState() => _RepoForksPageProviderState();
}
...
class _RepoForksPageProviderState extends State<RepoForksPageProvider> {
  ...
  void listener() {
    if (_controller.position.pixels == _controller.position.maxScrollExtent) {
      print(widget.name);
      BlocProvider.of<RepoBloc>(context).add(LoadForksEvent(
          name: widget.name, owner: widget.owner, isLoadNextForks: true));
    }
  }
